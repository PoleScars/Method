...
The films were deposited by an in house \acrshort{dc} magnetron sputtering facility (Figure \ref{fig:CaoSputtering}). This facility has a maximum power of 50 W, with a working gas of ultra-high purity Ar (99.999\%). The sputtering gun nominal target diameter is $25.4 mm$ ($1 in$). The working distance between target and substrate holder is $8 cm$. The chamber can achieve a base vacuum pressure of at least $10^{-2} Pa$, and a working Ar pressure of at least $2 Pa$ at constant flow rate of $2 cm^{3}/s$ ???\acrshort{sccm}???. The chamber has direct access and a large deposition area, allowing for batch processing of multiple substrates simultaneously.
...
\hl{START HERE ON MONDAY!!! There is a draft in the thesis but it was before doing any depositions so it will take some work to fix it. Look at deposition notes to fill in vague statements in the below paragraph}

A target was carefully affixed in the sputtering gun mount to ensure coincident, level contact with the copper holder. The desired substrate was position on the sample stage within the chamber and the chamber sealed. The chamber was evacuated to the desired base pressure, and then backfilled to the desired working Ar pressure. Coincident Ar pressure was maintained by a constant flow of Ar gas. This was necessary because the number of Ar$^{+}$ cations increases with chamber pressure, and accordantly increases the rate of deposition \cite{Ozeki2002}. Before deposition the targets were prepared by a pre-sputter to remove contamination and oxides from their surface. During this stage the substrates were protect by a shield positioned over the sample area preventing deposition.
...
All samples and targets were stored under Ar when not being examined or used. 
